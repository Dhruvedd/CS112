...
public class RUHungry {
     ...
     public void menu(String inputFile) {
        ...
        while(StdIn.hasNextLine()){  
            ...
            for(int j = 0; j < dishcount;j++){
                ...
                if(menuVar[i] == null) menuVar[i] = new MenuNode(dish, null);

                else{
                    MenuNode newDish = new MenuNode(dish, menuVar[i]);
                    menuVar[i] = newDish;
                }
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        StockNode gotcha = stockVar[index];

        for(StockNode ptr = stockVar[index];ptr != null; ptr = ptr.getNextStockNode()){
            if(ptr.getIngredient().getID() == ingredientID)gotcha = ptr;
        }
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        StockNode target = new StockNode(null, null);

        if(ingredientName == null) target = findStockNode(ingredientID);
        else target = findStockNode(ingredientName);
    ...
    public void updatePriceAndProfit() {
        ...
        for(int i = 0; i < menuVar.length; i++){
            ...
            for(MenuNode ptr = menuVar[i]; ptr != null; ptr = ptr.getNextMenuNode()){
                ...
                double dishProfit = dishPrice - dishCost;
    ...
    public void createStockHashTable (String inputFile){
        ...
        while(StdIn.hasNextLine()){
            ...
            StockNode newStock = new StockNode(ing, null);

            addStockNode(newStock);
        }
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        if(transactionVar == null) transactionVar = toAdd;
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        for(int i = 0; i < dish.getStockID().length; i++){
            ...
            int ingQuant = findStockNode(dish.getStockID()[i]).getIngredient().getStockLevel();

            if(ingQuant < numberOfDishes){
                isAvai = false;
            }

        }

        return isAvai; // update the return value
    }
